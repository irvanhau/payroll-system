...
@section('content')
    ...
    <div class="page-content">
        <div class="container-fluid">
            <div class="card">
                <div class="card-body">
                    <form method="post" action="javascript:void(0)" id="updateFormChartOfAccount">
                        @method('PUT')
                            ...
                            <div class="col-sm-10">
                                <select class="form-control" name="coa_category_id" id="coa_category_id">
                                    ...
                                    @foreach ($coa_categories as $coa_category)
                                        <option {{ $data->coa_category_id == $coa_category->id ? 'selected' : '' }}
                                            value="{{ $coa_category->id }}">{{ $coa_category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
    ...
    @push('addon-script')
        <script>
            $("#updateFormChartOfAccount").submit(function(e) {
                e.preventDefault();
